...
--트랜잭션 수행 중 갱신 결과를 바로 DB에 반영하는 기법은 무엇인가?  --즉각 갱신 회복 기법
    --회복기법(Recovery)이란?
        --트랜잭션을 수행하는 도중 장애로 인해 송상된 데이터베이스를 손상되기 이전의 정상적인 상태로 복구시키는 작업.
    --회복기법(Recovery)종류?
        --로그 기반 회복 기법: 1.자연 갱신 회복 기법(완료 전 DB기록 X)  2.즉각 갱신 회복 기법(완료 전 DB기록 O)
...
--프로시저와 사용자의 정의함수에서 파라미터를 입력받는 모드에 대한 설명이다. 빈칸을 작성하세요.
    ...
    --절차형 SQl(Procedural Language)이란? SQl언어에서도 절차 지향적인 프로그램이 가능하도록 하는 트랜잭션 언어.
    --절차형 SQL 종류
