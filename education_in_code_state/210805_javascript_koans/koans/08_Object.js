describe('Object에 대해서 학습합니다.', function () {
  ...
  */
  it('Object의 기본을 확인합니다.', function () {
    ...
    expect(emptyObj.length).to.equal(undefined);  // 객체는 length 프로퍼티를 참조할 수 없다고요 !!!! undefined
    ...
    expect(megalomaniac.length).to.equal(undefined);  // 객체는 length 프로퍼티를 참조할 수 없다 !!
    ...
    expect(megalomaniac.getMembers()).to.deep.equal(['Joker', 'Harley']);  // this 객체 이름 부른다 ?
  ...
  it('Object의 속성(property)를 다루는 방법을 확인합니다.', function () {
    ...
    expect('mastermind' in megalomaniac).to.equal(true);  // 왜 true ?
